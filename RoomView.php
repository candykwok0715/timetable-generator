...
_EOT1;
      ...
      do { 
	 	echo <<<_EOT2
      <option value='{$row_rs_r['roomNO']}'>{$row_rs_r['roomNO']}</option>
_EOT2;
      }while ($row_rs_r = mysql_fetch_assoc($rs_r));
echo "</select>";
    
echo <<<_EOT2
<input type="text" id="datepicker" class="changeWeek" name="inputDate" value="$date" /><br/>
...
foreach($TabletimeSlots as $value){
	...
	while($k < 5)
	{	
		$rs = "SELECT * FROM classes,modules WHERE date = '{$Week_cell[$k]}' AND tsfrom = '{$timeSlots[$i]}' and module = moduleCode and roomNo = '$room' ";
		//echo $rs;
		//echo '<br />';
